function App() {
    ...
    const ChangeTest = (event) => {
        ...
        console.log(`이동할페이지 : ${where}`);

        if (where === '-') {
            alert('이동할 페이지를 선택해주세요')
        } else {
            document.getElementById('myform').action = where; //이동할 페이지 정보변경
    ...
    const SubmitTest = (event) => {
        ...
        if (isChecked === true) {
            ...
            alert(`액션 페이지로 이동하지않습니다.`)
            event.preventDefault(); //이벤트 전파방지 (기본이동 방지)
        }
    }
    ...
    return (
        <div className="App">
            ...
            <form id='myform' onSubmit={SubmitTest} action={`https://www.naver.com`}>
                ...
                <input type="checkbox" id='go'></input>체크 상태일때만 해당 페이지로 이동
                <br /><br />
