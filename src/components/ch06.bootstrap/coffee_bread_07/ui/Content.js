...
function App({ contents, onClickToContent, categories, onOrderByClick, orderInfo, onChangeCategory }) {
    ...
    /*필드 검색 */
    /*넘겨 받은 카테고리 정보를 이용하여 동적으로 콤보박스를 생성합니다. */
    const comoCategory = categories.map((item, index) =>
        <option key={index} value={item.english}>{item.korean}</option>
    );

    /*사용자가 콤보 박스의 특정 카테고리 항목을 change 하였습니다. */
    const ChangedComboItem = (event) => {
        const changedCategory = event.target.value;
        //체인지와 관련된 프롭스에게 해당 카테고리 정보(영문이름)를 넘겨줍니다.
