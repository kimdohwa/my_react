function App() {
    ...
    const ClickEvent04 = (meg, event) => {
        const result = `파라미터 정보 : ${meg}`;
        console.log(result);
    ...
    const ClickEvent05 = (event) => {
        console.log(`해당 요소 id : ${event.target.id}`);
    ...
    return (
        <div className="App">
            <h2>클릭 이벤트</h2>
            {/*매개변수가 0개일때는 함수의 이름만 명시하면 됩니다. */}
            {/*해당 버튼을 클릭했을 때 ClickEvent01 함수가 동작 합니다. */}
            <button onClick={ClickEvent01}>매개 변수 0개</button><br />

            {/*매개변수가 있는 경우 화살표 함수로 둘러 싸야 합니다. */}
            <button onClick={() => ClickEvent02('김규식')}>매개 변수 1개</button><br />
            <button onClick={() => ClickEvent03(2, 3)}>매개 변수 2개</button><br />

            {/*이벤트 객체를 사용하고자 하는 경우*/}
            <button onClick={(event) => ClickEvent04('hello', event)}>이벤트 객체</button><br />

            {/*html 문서에서 요소를 구분하고자 할때는 id라는 속성을 사용합니다.*/}
