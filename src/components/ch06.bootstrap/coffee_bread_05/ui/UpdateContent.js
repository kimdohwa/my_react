...
/*등록 화면과의 차이점
1. 코멘트 : 등록 -> 수정으로 변경됨
2. 수정할 상품정보가 props로 넘어와야 합니다.
3. 각 입력 양식에 이전에 기입했던 정보를 미리 넣어 주어야 합니다.
    ㄴ 일반 양식은 value 속성에 명시하면 됩니다.
    ㄴ 콤보 박스는 옵션 괄호안에 selected={} 를 명시하면됩니다.
    ㄴ 라디오 버튼이나 체크박스는 기존 속성뒤에 checked를 명시하면 됩니다.
...
function App({ product, onSubmitUpdate, categories }) {
    ...
    const InputChange = (event) => {
        ...
        console.log(`파라미터 이름:${name}, 값 : ${value}`);
